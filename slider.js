export default class UI {
  constructor(sliderElement, leftArrowElement, rightArrowElement) {
    ...
    for (let index = 0; index < this.sliderElementsCount; index++) {
      ...
      const sliderElementImage = singleSliderElement.querySelector(
        ".slider__item-image"
      );
  ...
  moveLeft() {
    ...
    if (
      this.slideIndex ===
      this.sliderElementsCount - numberOfVisibleElements - 1
    )
      this.rightArrow.classList.add("slider__arrow--disabled");
    if (this.slideIndex === this.sliderElementsCount - numberOfVisibleElements)
      return;
    this.slideIndex++;
