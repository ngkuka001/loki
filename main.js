...
function init() {
  ...
  scene.background = new THREE.Color(
    configuration.colors.CanvasBackgroundColor
  );
  const near = 10;
  const far = 150;
...
function generateRandomObject(
  ...
  const randomWidth =
    availableSizes[0][randomIntFromInterval(0, availableSizes[0].length - 1)];
  const randomHeight =
    availableSizes[1][randomIntFromInterval(0, availableSizes[1].length - 1)];
  const randomColor =
    availableColors[randomIntFromInterval(0, availableColors.length - 1)];
...
function loadMain2DLetters() {
  const configurationLetters = document.querySelector(
